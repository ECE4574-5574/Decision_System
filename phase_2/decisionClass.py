...
class decisionMaking():
    def __init__(self,loggingLocation,server, deviceBase):
        self.logLocation = loggingLocation
    ...
    def deviceStateDecision(self,message):
        try:
            ...
            print "failing where it makes sense"
    ...
    def locationDecision(self, message):
        try:
            ...
            dateTimeObject = datetime.strptime(message["time"], "%Y-%m-%d %H:%M:%S")
            formatted = dateTimeObject.strftime("%Y-%m-%dT%H:%M:%SZ")
            handler = threading.Thread(None, self.decisionThread, 'Handler for POST/LocationChange', args = (message,formatted,self.storageAddress, self.deviceBaseAdd))
            print "thread start"
            handler.start() 
